    ...
    while activeControl and DoesEntityExist(currentVehicle) do
        ...
        if controlMode == 'standing' then
            if not IsEntityPlayingAnim(playerPed, Config.Animations.standing_control.dict, Config.Animations.standing_control.anim, 3) then
                ...
                TaskPlayAnim(playerPed, Config.Animations.standing_control.dict, Config.Animations.standing_control.anim,
                    8.0, -8.0, -1, Config.Animations.standing_control.flag, 0, false, false, false)
            end
        elseif controlMode == 'remote' then
            if not IsEntityPlayingAnim(playerPed, Config.Animations.remote.dict, Config.Animations.remote.anim, 3) then
                ...
                TaskPlayAnim(playerPed, Config.Animations.remote.dict, Config.Animations.remote.anim, 8.0, -8.0, -1,
                    Config.Animations.remote.flag, 0, false, false, false)
            end
...
function ActivateRemote(vehicle, vehicleName)
    ...
    -- Sende DIREKT NUI Message (nicht über ShowCompactHud)
    SendNUIMessage({
        action = 'showHud',
        vehicle = currentConfig
    })
    print('✅ Sent showHud message')
...
CreateThread(function()
    while true do
        ...
        -- Remote control
        if not inVehicle and Config.UseRemoteControl and not activeControl and not menuOpen then
            local vehicles = GetGamePool('CVehicle')
